...
import (
	...
	"data-platform-api-railway-line-creates-rmq-kube/config"
...
func (c *DPFMAPICaller) subfuncProcess(
	...
	for _, fn := range accepter {
		...
		case "ExpressType":
			c.expressTypeCreate(mtx, wg, subFuncFin, input, output, subfuncSDC, errs, log)
		case "StopStation":
			c.stopStationCreate(mtx, wg, subFuncFin, input, output, subfuncSDC, errs, log)
		case "DepartureStation":
			c.departureStationCreate(mtx, wg, subFuncFin, input, output, subfuncSDC, errs, log)
		case "DestinationStation":
			c.destinationStationCreate(mtx, wg, subFuncFin, input, output, subfuncSDC, errs, log)
		case "DptDstStation":
			c.dptDstStationCreate(mtx, wg, subFuncFin, input, output, subfuncSDC, errs, log)
		default:
			wg.Done()
		}
	}
...
func (c *DPFMAPICaller) expressTypeCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	defer wg.Done()
	err = c.complementer.ComplementExpressType(input, subfuncSDC, log)
	if err != nil {
		mtx.Lock()
		*errs = append(*errs, err)
		mtx.Unlock()
...
func (c *DPFMAPICaller) stopStationCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	defer wg.Done()
	err = c.complementer.ComplementStopStation(input, subfuncSDC, log)
	if err != nil {
		mtx.Lock()
		*errs = append(*errs, err)
		mtx.Unlock()
...
func (c *DPFMAPICaller) departureStationCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	defer wg.Done()
	err = c.complementer.ComplementDepartureStation(input, subfuncSDC, log)
	if err != nil {
		mtx.Lock()
		*errs = append(*errs, err)
		mtx.Unlock()
...
func (c *DPFMAPICaller) destinationStationCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	defer wg.Done()
	err = c.complementer.ComplementDestinationStation(input, subfuncSDC, log)
	if err != nil {
		mtx.Lock()
		*errs = append(*errs, err)
		mtx.Unlock()
...
func (c *DPFMAPICaller) dptDstStationCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	defer wg.Done()
	err = c.complementer.ComplementDptDstStation(input, subfuncSDC, log)
	if err != nil {
		mtx.Lock()
		*errs = append(*errs, err)
		mtx.Unlock()

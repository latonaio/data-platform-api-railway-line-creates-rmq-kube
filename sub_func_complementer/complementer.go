...
func (c *SubFuncComplementer) ComplementHeader(input *dpfm_api_input_reader.SDC, subfuncSDC *SDC, l *logger.Logger) error {
	s := &SDC{}
	numRange, err := c.ComplementRailwayLine(input, l)
	if err != nil {
		return xerrors.Errorf("complement railwayLine error: %w", err)
	}
...
func (c *SubFuncComplementer) ComplementRailwayLine(input *dpfm_api_input_reader.SDC, l *logger.Logger) (*NumberRange, error) {
	rows, err := c.db.Query(
		...
		WHERE (ServiceLabel, FieldNameWithNumberRange) = ( (?, 'RailwayLine') );`, input.ServiceLabel,
	)
	if err != nil {
	...
	nr.LatestNumber++
	input.Header.RailwayLine = nr.LatestNumber
	return &nr, nil
}
